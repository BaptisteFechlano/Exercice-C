...
void erase_graph(Graph g)
{
    ...
    //si sommets adjacents FONCTION LA PLUS IMPORTANTE pour lib mem
    if(g->tab_neighbours)
    {
        int i;
...
void add_edge(Graph g, int src, int dest)
{
    ...
    n->next = g->tab_neighbours[src-1].begin;  //commence par 1 et non pas par 0, autrement nous aurions noté [src]
...
void display_graph(Graph g)
{
    //WINDOWS SEULEMENT
    if(g->is_oriented)
        system("%CD%/graph/Graphviz/bin/dotty.exe digraph.out");//non portable ni sécurisé
    else
